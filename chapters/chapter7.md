...
웹 도구를 외부에 공개하면, 의도하지 않은 방식으로 사용되는 경우가 생긴다. 예를 들어:

- **SQL 인젝션**: 검색창에 일반 검색어 대신 데이터베이스 명령어를 입력하여, 데이터를 훔치거나 삭제하는 공격이다. 사용자 입력을 그대로 데이터베이스 쿼리에 넣으면 이런 문제가 발생한다.
- **XSS(크로스 사이트 스크립팅)**: 게시판이나 댓글에 악성 코드를 삽입하여, 다른 사용자의 브라우저에서 실행되게 하는 공격이다. 사용자 입력을 검증 없이 화면에 출력하면 위험하다.
- **명령 주입**: 10장에서 만들 BLAST 도구처럼, 사용자 입력을 시스템 명령에 전달하는 경우 악의적인 명령이 서버에서 실행될 수 있다.

완벽한 보안은 존재하지 않지만, AI를 활용하면 흔한 취약점을 상당 부분 잡아낼 수 있다. 기능 구현이 완료되면 Claude Code에게 보안 점검을 요청한다:

> 프로젝트 전체를 포괄적으로 보안 점검하고, 문제가 있으면 수정해줘

Claude Code가 코드를 분석하여 위와 같은 취약점을 찾아내고 자동으로 수정해 준다. 보안 용어를 몰라도 괜찮다 — AI가 어떤 문제가 있었는지, 왜 위험한지, 어떻게 수정했는지를 설명해 준다. 새로운 기능을 추가할 때마다 이 점검을 반복하는 습관을 들이면 좋다.
