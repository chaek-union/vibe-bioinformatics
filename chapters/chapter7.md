...
- **CORS 설정 미비**: CORS(Cross-Origin Resource Sharing)는 다른 도메인에서 우리 서버의 API를 호출할 수 있는지를 제어하는 설정이다. 이 설정이 `*`(모든 도메인 허용)으로 되어 있으면, 악의적인 외부 사이트에서 우리 API를 자유롭게 호출할 수 있다.
