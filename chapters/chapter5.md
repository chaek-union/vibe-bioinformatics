...
| **BAM/CRAM** | 참조 게놈에 정렬된 리드 | 정렬 결과 저장, 시각화 |
| **VCF** | 변이 정보 (위치, 유형, 빈도) | 변이 검출 결과 |
| **BED** | 게놈 구간 정보 (시작-끝 좌표) | 관심 영역(ROI) 정의 |

FASTQ 파일은 시퀀서에서 바로 나오는 원시 데이터이다. 각 리드마다 염기 서열과 품질 점수가 함께 기록되어 있다. BAM 파일은 이 리드들을 참조 게놈의 어디에 위치하는지 매핑한 결과이다. VCF 파일은 참조 게놈과 다른 부분, 즉 변이를 기록한 파일이다.
...
| **BWA-MEM2 / minimap2** | 리드를 참조 게놈에 정렬 | BWA-MEM2가 Illumina short read 표준 |
...
이 도구들은 대부분 Bioconda 채널을 통해 설치할 수 있다. 1장에서 설정한 micromamba를 활용하면 된다.

### Claude Code로 WGS 분석하기

실제 WGS 분석 과정을 Claude Code와 대화하며 진행하는 예시이다:

```text
> micromamba로 fastqc, fastp, bwa-mem2, samtools, gatk4, bcftools를 설치해줘
```
...
```text
> 트리밍된 리드를 GRCh38 참조 게놈에 BWA-MEM2로 정렬해줘. 정렬 후 samtools로 정렬, 인덱싱까지 해줘
```
...
| **정렬 (Alignment)** | 리드를 참조 게놈에 매핑하는 과정. BWA-MEM2가 표준 |
| **변이 검출 (Variant Calling)** | 참조 게놈과 다른 부분을 찾는 과정. GATK HaplotypeCaller가 표준 |
